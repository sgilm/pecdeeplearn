...
exp.create_experiment('double_a_conv_six_landmark')
exp.add_param('num_training_volumes', 41)
...
exp.add_param('landmark_4_dense_num_units', 1000)
exp.add_param('landmark_5_dense_num_units', 1000)
exp.add_param('landmark_6_dense_num_units', 1000)
...
banned = ['VL000' + str(index) for index in[90, 92, 93, 94, 95, 96]]
vol_list = [name for name in vol_list if name not in banned]
...
ext.add_feature(
    feature_name='landmark_4',
    feature_function=lambda volume, point:
    pdl.extraction.landmark_displacement(
        volume, point, exp.params['landmark_4'])
)
ext.add_feature(
    feature_name='landmark_5',
    feature_function=lambda volume, point:
    pdl.extraction.landmark_displacement(
        volume, point, exp.params['landmark_5'])
)
ext.add_feature(
    feature_name='landmark_6',
    feature_function=lambda volume, point:
    pdl.extraction.landmark_displacement(
        volume, point, exp.params['landmark_6'])
)
...
net = nolearn.lasagne.NeuralNet(
    layers=[
        ...
        (lasagne.layers.InputLayer,
         {'name': 'landmark_4', 'shape': (None, 3)}),
        (lasagne.layers.DenseLayer,
         {'name': 'landmark_4_dense',
          'num_units': exp.params['landmark_4_dense_num_units']}),
        (lasagne.layers.InputLayer,
         {'name': 'landmark_5', 'shape': (None, 3)}),
        (lasagne.layers.DenseLayer,
         {'name': 'landmark_5_dense',
          'num_units': exp.params['landmark_5_dense_num_units']}),
        (lasagne.layers.InputLayer,
         {'name': 'landmark_6', 'shape': (None, 3)}),
        (lasagne.layers.DenseLayer,
         {'name': 'landmark_6_dense',
          'num_units': exp.params['landmark_6_dense_num_units']}),
        ...
        (lasagne.layers.ConcatLayer,
         {'name': 'concat',
          'incomings': ['local_patch_flat', 'landmark_1_dense',
                        ...
                        'landmark_4_dense', 'landmark_5_dense',
                        'landmark_6_dense']}),
        (lasagne.layers.DenseLayer,
         {'name': 'join_dense',
